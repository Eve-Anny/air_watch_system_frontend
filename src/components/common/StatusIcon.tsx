import type { StatusRole } from "../../statusColors";

// Minimal inline SVGs (no icon-library dependency) - one glyph per status role, so status is never
// carried by color alone (dataviz skill non-negotiable).
export function StatusIcon({ role, className = "h-4 w-4" }: { role: StatusRole; className?: string }) {
  switch (role) {
    case "good":
      return (
